 ...
 * digests.
 */
public class VerificationHelpersCustom {
    ...
    private static final String TAG = VerificationHelpersCustom.class.getSimpleName();
        ...
        if (digestAlgorithm == DigestAlgorithm.SHA256) {
            if (publicKey.getKeyType() == KeyType.RSA) {
                ...
                try {
                    ...
                    //DEBUG_BYTE_ARRAY_D(TAG, "Extracted signature from interest:", signature);

//                    byte[] tempArray = new byte[buffer.position()];
//                    buffer.get(tempArray);
//                    DEBUG_BYTE_ARRAY_D(TAG, "Signed buffer of interest encoding:", tempArray);

                    //DEBUG_BYTE_ARRAY_D(TAG, "Key being used for verification:", publicKey.getKeyDer().getImmutableArray());
     ...
     WireFormat wireFormat)
    {
        ...
        //DEBUG_BYTE_ARRAY_D(TAG, "Extracted signature from interest:", signature.getSignature().getImmutableArray());

        SignedBlob encoding = interest.wireEncode(wireFormat);

        byte[] signedBufByteArrayForPrinting = new byte[encoding.signedSize()];
        encoding.signedBuf().get(signedBufByteArrayForPrinting);

        //DEBUG_BYTE_ARRAY_D(TAG, "Signed buffer of interest encoding:", signedBufByteArrayForPrinting);
